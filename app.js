...
import { logClientEvent, recordError, ensureProfile } from "./auth.js";

function escapeText(v) { return String(v ?? ""); }
function routeTo(p) { if (location.pathname.split("/").pop() !== p) location.href = p; }

onAuthStateChanged(auth, async user => {
  logClientEvent("info", "dashboard.auth.state", { uid: user?.uid || null });
  ...
  try {
    ...
    if (!snap.exists()) {
      logClientEvent("warn", "dashboard.profile.missing", { uid: user.uid });
      // Self-heal a rare auth/profile race or an older account with no profile.
      try {
        await ensureProfile(user);
        snap = await get(ref(db, `users/${user.uid}`));
      } catch (profileErr) {
        recordError("dashboard.profile.ensure.failed", profileErr, { uid: user.uid });
        return routeTo("signin.html");
      }
      if (!snap.exists()) {
        logClientEvent("error", "dashboard.profile.still_missing", { uid: user.uid });
        return routeTo("signin.html");
      }
      logClientEvent("info", "dashboard.profile.recovered", { uid: user.uid });
    }
    const data = snap.val();
  ...
  } catch (err) { recordError("dashboard.profile.load.failed", err, { uid: user.uid }); routeTo("signin.html"); }
});
...
document.getElementById("start-chat-form")?.addEventListener("submit", async e => {
  ...
  } catch (e2) { recordError("chat.lookup.failed", e2, { query: q }); err.textContent = e2.message || "Could not find that user."; }
});
...
document.getElementById("update-bank-form")?.addEventListener("submit", async e => {
  ...
  } catch (err) { recordError("profile.bank.update.failed", err, { uid: auth.currentUser?.uid || null }); msg.textContent = "Unable to update your account right now."; msg.style.display = "block"; }
});
