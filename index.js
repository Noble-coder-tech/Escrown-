...
admin.initializeApp({
  databaseURL: "https://escrown-5d946-default-rtdb.europe-west1.firebasedatabase.app"
});
...
exports.ensureUserProfile = onCall(async request => {
  const requestId = request.rawRequest?.headers?.['x-cloud-trace-context'] || `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  logger.info('ensureUserProfile.start', { requestId, uid: request.auth?.uid || null, hasUsername: Boolean(request.data?.username) });
  const uid = requireAuth(request);
  ...
  if (requestedUsername && (!/^[A-Za-z0-9_]{3,20}$/.test(requestedUsername) || RESERVED_USERNAMES.has(requestedUsername.toLowerCase()))) {
    logger.warn("ensureUserProfile.invalidUsername", { requestId, uid, username: requestedUsername });
  ...
  if (existingSnap.exists()) {
    ...
    logger.info("ensureUserProfile.existing.success", { requestId, uid, username, escrownId });
  ...
  if (!usernameResult.committed) {
    logger.warn("ensureUserProfile.usernameTaken", { requestId, uid, username: usernameKey });
  ...
  try {
    ...
    logger.info("ensureUserProfile.new.success", { requestId, uid, username, escrownId });
    return profile;
  } catch (err) {
    logger.error("ensureUserProfile.new.failed", { requestId, uid, username, escrownId, error: err?.message, stack: err?.stack });
    try { await usernameRef.remove(); } catch (cleanupErr) { logger.error("ensureUserProfile.usernameCleanup.failed", { requestId, uid, username, error: cleanupErr?.message }); }
    if (escrownId) { try { await db.ref(`escrown_ids/${escrownId}`).remove(); } catch (cleanupErr) { logger.error("ensureUserProfile.escrownIdCleanup.failed", { requestId, uid, escrownId, error: cleanupErr?.message }); } }
    throw new HttpsError("internal", "We could not finish creating your account. Please try again.");
  }
});
