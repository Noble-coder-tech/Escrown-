...
async function call(name, data) { try { const result = (await httpsCallable(functions, name)(data)).data; logClientEvent("info", `call.${name}.success`); return result; } catch (err) { recordError(`call.${name}.failed`, err, { data }); throw err; } }
